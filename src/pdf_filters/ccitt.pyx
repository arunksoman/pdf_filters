...
cdef class BitParser:
    ...
    @classmethod
    cdef public void add(list root, IntOrStr v, str bits):
        cdef list p = root
        cdef int b, i
        cdef int bits_len = len(bits)
    ...
    cdef public void _parse_bit(self, object x):
        cdef object v
        if x:
            v = self._state[1]
        else:
...
cdef class CCITTG4Parser(BitParser):
    cdef public int width
    cdef public bint bytealign
    cdef public array.array _curline
    cdef public array.array _refline
    cdef int _y
    cdef int _curpos
    cdef int _color
    cdef int _n1
    ...
    def __init__(self, int width, bint bytealign = False):
    ...
    cdef public void output_line(self, int y, array.array bits):
        print(y, "".join(str(b) for b in bits))
    
    cdef public void _reset_line(self):
        self._refline = self._curline
        self._curline = array.array("b", [1] * self.width)
        self._curpos = -1
        self._color = 1

    cdef public void _flush_line(self):
